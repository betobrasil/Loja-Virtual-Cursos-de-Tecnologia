...
function cadastrarProduto($nomeProduto,$descricaoProduto,$imgProduto,$precoProduto){
    ...
    //  A função file_exists é para ver se o arquivo existe. A função isset é para saber se o conteúdo da variável existe.
    if(file_exists($nomeArquivo)){
        ...
        // Salvando os dados no arquivo "produtos.json". 
        // Sintaxe da Estrutura - primeiro nome do arquivo, depois o encode (variável que faz o encode). (PUT=colocar)
        $deuCerto = file_put_contents($nomeArquivo,$json);

            if($deuCerto){
                return "Deu certo, produto adicionado no arquivo [produtos.json].";
            }else{
                return "Não deu certo, o produto NÃO foi adicionado no arquivo.";
            }
        var_dump($produtos);
        
    }else{
        // $produtos - mesmo nome da variável definida no arquivo variaveis.php
        ...
        $deuCerto = file_put_contents($nomeArquivo,$json);
            ...
            if($deuCerto){
                return "Deu certo ! O arquivo produtos.json foi criado.";
            }else{
